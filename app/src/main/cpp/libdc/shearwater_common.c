/*
 * DeepLog: modified copy of libdivecomputer's src/shearwater_common.c at 415778c.
 *
...
shearwater_common_download (shearwater_common_device_t *device, dc_buffer_t *buffer, unsigned int address, unsigned int size, unsigned int compression, dc_event_progress_t *progress)
{
	...
	// DeepLog: a well-formed refusal of the init request is the device
	// saying it will not serve that address — not corruption on the link.
	// Upstream reports it as DC_STATUS_PROTOCOL, which is indistinguishable
	// from a garbled packet, so the Petrel driver could only abort. Report
	// it as DC_STATUS_UNSUPPORTED instead, mirroring how rdbi below already
	// treats a NAK, so the caller can decide whether the refusal is fatal.
	if (n == 3 && response[0] == NAK && response[1] == req_init[0]) {
		WARNING (abstract->context, "Device refused download of %u bytes at 0x%08x (NAK 0x%02x).",
			size, address, response[2]);
