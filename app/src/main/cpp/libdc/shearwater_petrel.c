/*
 * DeepLog: modified copy of libdivecomputer's src/shearwater_petrel.c at 415778c.
 *
 * CMakeLists.txt compiles this file in place of the submodule's copy, so the
 * fixes live in this repository and survive a fresh clone and CI, instead of
 * being uncommitted edits inside the submodule. Every change is marked with a
 * "DeepLog:" comment; everything else is upstream, verbatim. When bumping the
 * submodule, diff the upstream file against this one and carry the changes
 * across.
 *
 * Why these exist: dives were going missing when downloading from a
 * Shearwater Perdix. See the commit that introduced this file.
 */
...
shearwater_petrel_device_foreach (dc_device_t *abstract, dc_dive_callback_t callback, void *userdata)
{
	...
	while (1) {
		...
		// DeepLog: another page is only requested when the previous one was
		// completely full, but a full last page looks exactly like one with
		// more to follow. When the log ends on a page boundary the device
		// refuses the extra request, and that refusal is the end of the
		// manifest, not an error. Upstream aborted here, losing every dive.
		// A refused *first* page is still an error: nothing has been read.
		if (rc == DC_STATUS_UNSUPPORTED && page > 0) {
			INFO (abstract->context, "Manifest ends on a page boundary after %u page(s).", page);
			maximum -= 1 + RECORD_COUNT;
		...
		// Append every record the walk passed over, deleted ones included.
		//
		// DeepLog: upstream appended only count * RECORD_SIZE bytes, but the
		// walk steps over deleted records without counting them. So a deleted
		// record ahead of valid ones stayed inside that prefix while the last
		// valid record(s) fell off its end: each dive deleted on the computer
		// silently cost one real dive. `offset` is exactly the span walked;
		// the download loop below already skips the deleted records.
	...
	unsigned int offset = 0;
	unsigned int attempted = 0, refused = 0;
	while (offset < size) {
		// skip deleted dives
		...
		// DeepLog: a dive the device refuses to serve is one whose record
		// points at data it no longer holds. Retrying cannot help, and
		// upstream's abort made every older dive unreachable on every
		// attempt, forever. Skip it and keep going. A timeout or I/O error
		// is different — the link is failing — and still aborts, because
		// pressing on would only turn one lost dive into many slow ones.
		if (rc == DC_STATUS_UNSUPPORTED) {
			refused++;
			WARNING (abstract->context, "DEEPLOG_SKIPPED_DIVE record=%u address=0x%08x",
				offset / RECORD_SIZE, base_addr + address);
			rc = DC_STATUS_SUCCESS;
			current += 1;
			offset += RECORD_SIZE;
			continue;
	...
	// DeepLog: one refused dive is a bad record; every dive refused is
	// something systemic, such as reading from the wrong logbook address.
	// Reporting success with nothing downloaded would hide that entirely.
	if (attempted > 0 && refused == attempted) {
		ERROR (abstract->context, "The device refused all %u dives.", attempted);
